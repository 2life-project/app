...
Наружу из фичи выходят экран и панель (`BandPanel`, `BandScreen`) и четыре
секции для разделов приложения — `BandHeartSection`, `BandBreathingSection`,
`BandRecoverySection`, `BandActivitySection` (см. 3.13).
...
| `model/publish-readings.ts` | показания дня — остальному приложению через `shared/domain`    |
| `model/profile-sync.ts`     | профиль тела на устройство: перевод в поля протокола           |
| `model/on-sign-out.ts`      | выход из аккаунта: снимок, архив и ночи уходят с человеком     |
