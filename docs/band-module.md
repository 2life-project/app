...
Пока приложение подключено, устройство шлёт отчёт примерно **раз в десять
секунд**, без запроса.

**Это накопитель незавершённой минуты, а не готовая минута.** Счётчики
сбрасываются на её границе: отчёт в 14:32:20 несёт `steps: 40`, тогда как итог
минуты будет 118.

Поэтому у живого слота `source: "live"`, отметка округляется до начала минуты, и
**при совпадении история всегда побеждает живой отчёт**. Между двумя живыми
побеждает тот, у которого счётчики больше.

Без этого правила последние минуты каждого сеанса остаются занижены навсегда:
история перечитывается только за сегодня.
