...
### 3.13. Показания для остального приложения
...
**Профиль тела** живёт там же и уезжает на устройство при каждом подключении:
команды чтения профиля в протоколе нет, сверить записанное нечем.
...
- **Идентификатор события устойчив**: считается из адреса устройства, потока и
  естественного ключа события (минута, дата, начало ночи). Прочитанное второй
  раз уезжает под тем же именем.
...
- **Аудио** отправленным считается только по `audioStored: true`. Файл
  регистрируется с SHA-256, грузится частями по пределу сервера, докачивается
  по его же списку принятых частей. Метки уезжают и с заявкой, и своим потоком:
  файл могут не скачать вовсе, а нажатие кнопки существует только в отчёте.
