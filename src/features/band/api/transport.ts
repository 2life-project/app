    ...
    this.notifications = this.device.monitorCharacteristicForService(
      ...
      (error, characteristic) => {
        if (error) {
          ...
          // а браслет остаётся «подключённым» навсегда и молча. Если закрыли
          // мы сами (`stop`), это не событие: стек сообщает об отмене подписки
          // тем же путём, что и о потере связи.
          if (!this.gone)
