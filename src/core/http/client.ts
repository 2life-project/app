...
async function unwrap<T>(path: string, response: Response): Promise<T> {
  ...
  if (!response.ok) {
    ...
    // строкой: вложенный объект консоль печатает как [Object]. Уровень —
    // предупреждение: сервер ответил, и отказ ли это, решает вызывающий; для
    // очереди браслета конфликт записи штатен.
    logger.warn('Запрос не прошёл', {
      path,
